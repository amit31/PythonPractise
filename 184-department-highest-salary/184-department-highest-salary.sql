...
from
(select e.name as employeename,d.name ,salary , rank () over ( partition by departmentID order by salary desc) as dr from employee e join department d on e.departmentId=d.id) temp where temp.dr=1